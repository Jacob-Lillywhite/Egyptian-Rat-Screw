<?php
	...
	for(;$numberOfPlayers >= 0; $numberOfPlayers--){
		...
		//check if they are the turn owner
	...
	if($decksWithCards > 1){
	 //or $currentOwner != $winner or (substr($cards[0],0, strlen($cards[0])-1) == substr($cards[1],0, strlen($cards[1])-1) and $decks[0] > 1)){
		$winner = 0;
	...
	/*if($turnOwner == $currentOwner and (count($cards) < 1 or substr($cards[0],0, strlen($cards[0])-1) != substr($cards[1],0, strlen($cards[1])-1))){
		$returnValue["winner"] = $currentOwner;
	}else{
		$returnValue["winner"] = 0;
	}*/	
